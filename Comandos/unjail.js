module.exports = {
  nombre: "desencarcelar",
  alias: ["fianza", "unjail"],
  cooldown: 2000,
  descripcion: "Desencarcela a un miembro",
  uso: "[miembro] (razón)",
  categoria: "mod",
  run: (Discord, client, message, args) => {
    ...
    if (!message.member.permissions.has(Discord.Permissions.FLAGS.BAN_MEMBERS)) {
      ...
      embed.setDescription("No puedes desencarcelar a este usuario")
      return mensajerror()
    } else {
      embed.setDescription(`**${miembro.user.tag} desencarcelado** || Razón - ${razon}`)
      miembro.roles.remove("849456488858910740", razon).then(() => {
